...
const { getGlobalSymlinks, globalLinkPath, getNodeModulePath } = require("npm-link-watch-common");
...
function unlinkPerPackage(packageName) {
    const restoredFiles = getGlobalSymlinks(packageName).map(file => fs.readlinkSync(file));
